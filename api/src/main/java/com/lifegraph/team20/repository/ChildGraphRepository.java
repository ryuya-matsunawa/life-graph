...
public class ChildGraphRepository {
  ...
  public void updateChildByUserIdAndAge(long userId, int age, int score, String comment) {
    final String parentId = "( select id from parent_graphs where user_id = " + userId + ") ";
    jdbcTemplate.update("update child_graphs set age = " + age + ", score = " + score + ", comment = '" + comment
        + "' where age = " + age + " and " + parentId);
  }

  public void updateChildById(long id, int age, int score, String comment) {
    jdbcTemplate.update("update child_graphs set age = " + age + ", score = " + score + ", comment = '" + comment
        + "' where id = " + id);
  }
